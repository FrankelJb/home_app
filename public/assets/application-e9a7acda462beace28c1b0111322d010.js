...
(function() {
  $(function() {
    var replacements_backward, replacements_forward;
    replacements_backward = {
      0: 'o',
      1: 'i',
      2: 'z',
      3: 'e',
    ...
    $('#password_numbers_for_letters').click(function() {
      var key, re, value, word;
      word = $('#password_input').val();
      if (!$('#password_expires_in_three_months').is(':checked')) {
        $('#alert-text').html('');
        if ($('#password_numbers_for_letters').is(':checked')) {
          for (key in replacements_forward) {
            value = replacements_forward[key];
            re = new RegExp(key, "g");
            word = word.replace(re, value);
          }
        } else {
          for (key in replacements_backward) {
            value = replacements_backward[key];
            re = new RegExp(key, "g");
            word = word.replace(re, value);
          }
        }
        return $('#password_input').val(word);
      } else {
        return $('#alert-text').html('Date may become numbers');
      }
    });
    $('#password_camel_case').click(function() {
      var c, index, indices, new_word, word, _i, _ref;
      word = $('#password_input').val();
      if (!$('#password_camel_case').is(':checked')) {
        return $('#password_input').val(word.toLowerCase());
      } else {
        indices = $('#indices').text().split(',');
        new_word = '';
        index = 0;
        for (c = _i = 0, _ref = word.length; 0 <= _ref ? _i < _ref : _i > _ref; c = 0 <= _ref ? ++_i : --_i) {
        ...
        return $('#password_input').val(new_word);
      }
    });
    $('#password_keep_spaces').click(function() {
      var characters, count, i, index, indices, new_word, word, _i, _j, _ref, _ref1;
      word = $('#password_input').val();
    ...
    return $('#password_expires_in_three_months').click(function() {
      var current_date;
      if (!$('#password_expires_in_three_months').is(':checked')) {
        return $('#password_input').val($('#password_input').val().substring(0, $('#password_input').val().length - 6));
      } else {
        current_date = new Date();
        current_date.setMonth(current_date.getMonth() + 3);
        return $('#password_input').val($('#password_input').val() + ("0" + (current_date.getMonth() + 1)).slice(-2) + current_date.getFullYear());
      }
    });
  });
